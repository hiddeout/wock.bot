...
tiktok_video1 = re.compile(r"(?:http\:|https\:)?\/\/(?:www\.)?tiktok\.com\/@.*\/(?:photo|video)\/\d+")
tiktok_video2 = re.compile(r"(?:http\:|https\:)?\/\/(?:www|vm|vt|m).tiktok\.com\/(?:t/)?(\w+)")
...
class TikTokClient:
    def __init__(self, ms_tokens: Union[List[str], str], sessions: int, sleep_after: int):
    ...
    async def setup(self: Self) -> None:
        ...
        for token in self.ms_tokens:
            session = TikTokAPI()
            await session.create_sessions(ms_tokens = [token], num_sessions = self.sessions, sleep_after = self.sleep_after)
        ...
        async with ClientSession() as session:
            async with session.get(url, headers = HEADERS) as response:
                if response.status != 200:
                    raise TypeError(f"response got a status of {response.status} for {url}")
                kwargs["data"] = await response.read()
        kwargs["file"]["filename"], kwargs["file"]["extension"] = self.get_extension(url)
        return Asset(**kwargs)
    ...
    async def get_user_feed(self: Self, username: str, count: int) -> Optional[UserFeed]:
        await self.check()
        session = next(self.clients)
        @ratelimit("tiktok_request:{session}", 2, 5, True)
        @lock("tiktok_request:{session}")
    ...
    async def get_video(self: Self, url: str, with_obj: bool = True) -> Optional[Union[OldVideo, Tuple[OldVideo, Any]]]:
        ...
        async with aiohttp.ClientSession() as session:
            async with session.request("OPTIONS", "https://api22-normal-c-alisg.tiktokv.com/aweme/v1/feed/", params = params) as response:
