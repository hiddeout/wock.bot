...
class Errors(Cog):
    ...
    async def handle_exceptions(self, ctx: Context, exception: Exception) -> None:
        ...
        if isinstance(exception, commands.BadUnionArgument):
            ...
            if await self.bot.glory_cache.ratelimited(
                f"rl:cooldown_message{ctx.author.id}", 1, 5
            ):
                return
        ...
        if isinstance(exception, commands.MissingPermissions):
            ...
            return await ctx.warning(
                f"Must have **{', '.join(missing_permissions)}** permissions"
            )
        if isinstance(exception, commands.BotMissingPermissions):
            ...
            return await ctx.warning(
                f"I Must have **{', '.join(missing_permissions)}** permissions"
            )
        if isinstance(exception, commands.MissingRequiredArgument):
            if await self.bot.glory_cache.ratelimited(
                ...
                return
            return await ctx.warning(get_message(exception.param.name))
        if isinstance(exception, commands.BadArgument):
            error = exception
            tb = "".join(
                traceback.format_exception(type(error), error, error.__traceback__)
