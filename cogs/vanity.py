...
from tools.important.subclasses.command import (
    Role,
    TextChannel
)
...
class Vanity(commands.Cog):
    ...
    async def cog_load(self):
        await self.bot.db.execute("""CREATE TABLE IF NOT EXISTS vanity_roles (guild_id BIGINT NOT NULL, user_id BIGINT NOT NULL, PRIMARY KEY(guild_id, user_id))""")
        logger.info("Now starting the check vanity loop...")
        self.check_vanity.start()
        logger.info(
            "Started the check vanity loop!"
        )
    ...
    async def award_message(self, member: discord.Member):
        data = await self.bot.db.fetchrow("""SELECT channel_id, message FROM vanity_status WHERE guild_id = $1""", member.guild.id)
        if not data:
            return
    ...
    async def assign_vanity_role(self, member: discord.Member, role: discord.Role):
        ...
        if role in member.roles:
            return
        await self.bot.db.execute("""INSERT INTO vanity_roles (guild_id, user_id) VALUES($1, $2) ON CONFLICT(guild_id, user_id) DO NOTHING""", guild.id, member.id)
    ...
    async def remove_vanity_role(self, member: discord.Member, role: discord.Role):
        await self.bot.db.execute("""DELETE FROM vanity_roles WHERE guild_id = $1 AND user_id = $2""", member.guild.id, member.id)
        guild = member.guild
            ...
            if member.status != discord.Status.offline:
                if vanity in self.activity(member):
                    ...
                    if role := await self.get_vanity_role(member.guild):
                        if role in member.roles:
                            if not await self.bot.db.fetchrow("""SELECT * FROM vanity_roles WHERE guild_id = $1 AND user_id = $2""", member.guild.id, member.id):
                                return
    ...
    @commands.group(name="vanity", brief = "reward users with a role for repping the vanity", example=",vanity", invoke_without_command = True)
    @commands.has_permissions(manage_roles=True)
    async def vanity(self, ctx: Context):
        return await ctx.send_help(ctx.command.qualified_name)

    @vanity.command(name="role", brief = "set the reward role", example = ",vanity role @pic")
    @commands.has_permissions(manage_roles=True)
    ...
    @vanity.group(name = "award", brief = "add a message into a channel upon someone repping", invoke_without_command = True)
    @commands.has_permissions(manage_roles=True)
    async def vanity_award(self, ctx: Context):
        return await ctx.send_help(ctx.command.qualified_name)
    
    @vanity_award.command(name = "message", brief = "set the message", example = ",vanity award message {embed}{description: thanks for repping {user.mention}}")
    @commands.has_permissions(manage_roles=True)
    async def vanity_award_message(self, ctx: Context, *, message: EmbedConverter):
        try:
            await self.bot.db.execute("""UPDATE vanity_status SET message = $2 WHERE guild_id = $1""", ctx.guild.id, message)
        except:
            return await ctx.fail(f"**Vanity role needs to be set** with `{ctx.prefix}vanity role`")
        return await ctx.success("**Vanity Award message** has been set")
    
    @vanity_award.command(name = "channel", brief = "set the award message channel", example = "vanity award channel #text")
    @commands.has_permissions(manage_roles=True)
    async def vanity_award_channel(self, ctx: Context, *, channel: TextChannel):
        try:
            await self.bot.db.execute("""UPDATE vanity_status SET channel_id = $2 WHERE guild_id = $1""", ctx.guild.id, channel.id)
        except:
            return await ctx.fail(f"**Vanity role needs to be set** with `{ctx.prefix}vanity role`")
        return await ctx.success(f"**Vanity award channel** to {channel.mention}")
    
    @vanity.command(name = "view", aliases = ["config", "cfg", "settings"], brief = "view your vanity status settings")
    @commands.has_permissions(manage_roles=True)
    async def vanity_view(self, ctx: Context):
        data = await self.bot.db.fetchrow("""SELECT role_id, channel_id, message FROM vanity_status WHERE guild_id = $1""", ctx.guild.id)
        if not data:
        ...
        embed = discord.Embed(title = "vanity status config", color = self.bot.color, description = desc)
