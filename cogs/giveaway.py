...
from discord.ext.commands import Context, Cog, has_permissions  # type: ignore # type: ignore
...
class Giveaway(Cog):
	...
	async def fetch_message(self, guild: Union[int, discord.Guild], channel_id: int, message_id: int) -> Optional[discord.Message]:
			...
			if guild := self.bot.get_guild(int(gw.guild_id)):
				...
				if gw.expiration.timestamp() < datetime.now().timestamp():
					...
					if channel := guild.get_channel(int(gw.channel_id)):
						message = await self.fetch_message(channel.guild, channel.id, int(gw.message_id))
						is_fetched = True
						try:
							prize = (
